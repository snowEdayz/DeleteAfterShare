...
编辑截图后通过“分享”发送时，模块把编辑前原图 URI 放进截图应用发往图库的分享 Intent。图库进程在现有 `ShareInnerViewModel.e0(Set)` 调用点，把原图对应的图库路径加入同一次待删除队列调用。
...
Hook 使用的是原始 DEX 成员名。JADX 输出中的 `m33828e0`、`getImageInfo` 等名称可能是反编译器为避免冲突或根据 Kotlin 元数据生成的阅读名称，不能直接作为 Hook 名。
